...
import (
	...
	etrace "git.4321.sh/feige/flygo/core/ktrace"
	"git.4321.sh/feige/flygo/core/ktrace/otel"
...
func (o *Flygo) initTracer() error {
	var (
		container *otel.Config
	)

	if kcfg.Get(o.opts.configPrefix+"trace") != nil {
		container = otel.Load(o.opts.configPrefix + "trace")
	} else {
		container = otel.DefaultConfig()
	}

	// 禁用trace
	if kcfg.GetBool(o.opts.configPrefix + "trace.disable") {
		klog.FlygoLogger.Info("disable trace", klog.FieldComponent("app"))
		return nil
	}

	tracer := container.Build()
	etrace.SetGlobalTracer(tracer)
	o.opts.afterStopClean = append(o.opts.afterStopClean, container.Stop)
	klog.FlygoLogger.Info("init trace", klog.FieldComponent("app"))
	return nil
}
